...
public class Succursale extends UnicastRemoteObject implements SuccursaleInterface, Runnable {
	...
	private void printSuccursales()
	{
		if(_succursales.size() == 0)
			System.out.println("Aucune succursale connectÃ©e.");
		else
			System.out.println("Succursales connectÃ©es:");
		...
		while (true)
		{
			try
			{
				if(_montant > 0 && _succursales.size() > 0)
				{
					...
					System.out.println("TransfÃ¨re " + transferAmount + "$ Ã  la succursale #" + randomSuccursale.getIdentity());
					System.out.println(this);
